...
// testing nbk: survey-specific NB over-dispersion
// testing zip: survey-specific zero-inflated NB
...
Type objective_function<Type>::operator() (){
	...
	for(int i_site = 0; i_site < nsite; i_site++){
		...
		if(Options(1) == 0){
			nll(2) -= dnbinom2(catch_site, mu_site, mu_site+mu_site*mu_site/nbk_site, true);
		}else if(Options(1) == 1){
			nll(2) -= dzinbinom2(catch_site, mu_site, mu_site+mu_site*mu_site/nbk_site, zip_site, true);
		}
	}
