...
# read data
data <- read.csv("../../Data/20190510-Adam-Survey/ComparativeData.csv") %>%
  transmute(stn = STATION, len = Length, NEST = NEST, BALL = BALL)
...
d.orig <- read.csv("../../Data/20190510-Adam-Survey/ComparativeData.csv") %>%
  transmute(station = STATION, len = Length, A = NEST, B = BALL) %>% 
...
# read data
d <- read.csv("../../Data/20190510-Adam-Survey/ComparativeData.csv") %>%
  transmute(station = STATION, len = Length, NEST = NEST, BALL = BALL) %>% 
  gather(gear, catch, -station, -len) %>%
  filter(catch > 0) %>%
  transmute(
