...
Type objective_function<Type>::operator() () {
	...
	DATA_IVECTOR(ind); // grouping index
	DATA_INTEGER(n_ind); // number of groupings
	...
	for (int i_ind = 0; i_ind < n_ind; i_ind++){
		vector<Type> tmp_delta = delta.row(i_ind);
		nll(2) += MVNORM(C_delta)(tmp_delta);
		vector<Type> tmp_epsilon = epsilon.row(i_ind);
	...
	for(int i_s = 0; i_s < n_s; i_s++){
		for(int i_len = 0; i_len < n_len; i_len++){
			// linear predictors
			for(int i_f = 0; i_f < n_f; i_f++){
				eta_mu(i_s, i_len) += Xf(i_len, i_f) * (beta(i_f) + delta(ind(i_s)-1, i_f));
				eta_phi(i_s, i_len) += Xf(i_len, i_f) * gamma(i_f);
			}
			for(int i_r = 0; i_r < n_r; i_r++){
				eta_mu(i_s, i_len) += Xr(i_len, i_r) * (b(i_r) + epsilon(ind(i_s)-1, i_r));
